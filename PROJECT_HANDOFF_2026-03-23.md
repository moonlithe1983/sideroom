...
- As of this handoff, the expected red items are still missing real Supabase env values plus placeholder policy, terms, support, and marketing metadata in `config/release-metadata.json`, while `seed-authors.json` remains a launch-content warning rather than a repo-breaking failure.
...
- Placeholder privacy-policy, terms, support, and marketing values still need to be replaced in `config/release-metadata.json`.
...
7. Use `docs/device-smoke-checklist.md` to test auth, onboarding, posting, moderation, inbox, restrictions, and trust surfaces on real devices.
8. Replace the placeholder privacy-policy, terms, support, and marketing values in `config/release-metadata.json`.
